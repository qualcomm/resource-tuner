...
class ResourceTunerServerLifecycleTests:
    def __init__(self):
        pass
    ...
    def TestResourceTunerServerBootupAndTermination(self):
        logger = logging.getLogger()
        logger.info(self.TestResourceTunerServerBootupAndTermination.__name__ + ": STARTED")
        testResult = True

        pid = os.fork()
        if pid == 0:
            time.sleep(1)
            os.execvp("./resource_tuner", ["./resource_tuner", "--start"])
        elif pid > 0:
            time.sleep(3)
        ...
        if testResult == True:
            logger.info(self.TestResourceTunerServerBootupAndTermination.__name__ + ": PASSED")

        return testResult

    def TestResourceTunerServerCrashRecovery(self):
        logger = logging.getLogger()
        logger.info(self.TestResourceTunerServerCrashRecovery.__name__ + ": STARTED")
        testResult = True
        ...
        if testResult == True:
            logger.info(self.TestResourceTunerServerCrashRecovery.__name__ + ": PASSED")
...
if __name__ == "__main__":
    ...
    testObj = ResourceTunerServerLifecycleTests()

    assert testObj.TestResourceTunerServerBootupAndTermination() == True, "Test: TestResourceTunerServerBootupAndTermination Failed"
    assert testObj.TestResourceTunerServerCrashRecovery() == True, "Test: TestResourceTunerServerCrashRecovery Failed"

    logger = logging.getLogger()
    logger.info("All Tests part of the ResourceTunerServerLifecycleTests suite, Successfully Passed")
