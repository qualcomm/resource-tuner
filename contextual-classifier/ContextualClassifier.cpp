    ...
    while (true) {
        int pid_to_classify = -1;
        {
            ...
            while (!mClassificationQueue.empty()) {
                ...
                // else: PID is no longer pending (EXIT already received), skip
                // it
            }
        }
...
void ContextualClassifier::ClassifyProcess(
    int process_pid, int process_tgid,
    std::unordered_map<int, int> &pid_perf_handle) {
    (void)process_tgid;
    ...
    if (access(proc_path.c_str(), F_OK) == -1) {
        LOGD(CLASSIFIER_TAG,
             format_string("Process %d has exited.", process_pid));
        return;
    }
    ...
    FeatureExtractor::CollectAndStoreData(process_pid, mTokenIgnoreMap,
                                          raw_data, mDebugMode);
    ...
    LOGD(CLASSIFIER_TAG, format_string("Data collection for PID:%d took %f ms",
                                       process_pid, elapsed_collect.count()));
    ...
    if (has_sufficient_features) {
        ...
        LOGD(CLASSIFIER_TAG, format_string("Skipping ML inference for PID:%d "
                                           "due to insufficient features.",
                                           process_pid));
    }
}

void ContextualClassifier::RemoveActions(
    int process_pid, int process_tgid,
    std::unordered_map<int, int> &pid_perf_handle) {
    (void)process_tgid;
...
int ContextualClassifier::HandleProcEv() {
    ...
    while (!mNeedExit) {
        ...
        case PROC_EVENT_FORK:
            LOGD(CLASSIFIER_TAG,
                 format_string(
                     "fork: parent tid=%d pid=%d -> child tid=%d pid=%d",
        ...
        case PROC_EVENT_EXEC:
...

            {
                ...
                // Simple check for ignore list (logic duplicated from
                // ClassifyProcess but fast path)
                ...
                if (comm_file.is_open()) {
                    ...
                    LOGD(
                        CLASSIFIER_TAG,
                        format_string(
                            "Process %d exited before initial check. Skipping.",
                            pid));
                }
            }
            break;
