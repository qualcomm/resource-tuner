...
std::vector<std::string> FeaturePruner::splitString(const std::string& input, const std::string& delimiters) {
...
std::string FeaturePruner::removeDatesAndTimesFromToken(const std::string& input) {
...
std::vector<std::string> FeaturePruner::filterStrings(const std::vector<std::string>& input, const std::unordered_set<std::string>& ignoreSet) {
    std::vector<std::string> result;
...
std::vector<std::string> FeaturePruner::toLowercaseVector(const std::vector<std::string>& input) {
...
static const std::regex kUuidRe(R"(\b[0-9a-fA-F]{8}(?:-[0-9a-fA-F]{4}){3}-[0-9a-fA-F]{12}\b)");
...
void FeaturePruner::normalize_numbers_inplace(std::vector<std::string>& tokens) {
    for (auto& s : tokens) {
        s = replace_numbers_and_hex_with_N(s);
    }
}

std::unordered_map<std::string, std::unordered_set<std::string>> FeaturePruner::loadIgnoreMap(const std::string& filename, const std::vector<std::string>& labels) {
