...
int FeatureExtractor::collect_and_store_data(pid_t pid, const std::unordered_map<std::string, std::unordered_set<std::string>>& ignoreMap, std::map<std::string, std::string>& output_data, bool dump_csv) {
    ...
    std::vector<std::string> lowercmdline = FeaturePruner::toLowercaseVector(cmdline);
    auto filtered_cmd = FeaturePruner::filterStrings(lowercmdline, ignoreMap.count("cmdline") ? ignoreMap.at("cmdline") : std::unordered_set<std::string>());
    ...
    auto filtered_maps = FeaturePruner::filterStrings(lowermaps, ignoreMap.count("map_files") ? ignoreMap.at("map_files") : std::unordered_set<std::string>());
    ...
    auto filtered_fds = FeaturePruner::filterStrings(lowerfds, ignoreMap.count("fds") ? ignoreMap.at("fds") : std::unordered_set<std::string>());
    ...
    std::vector<std::string> lowerenviron = FeaturePruner::toLowercaseVector(environ);
    auto filtered_environ = FeaturePruner::filterStrings(lowerenviron, ignoreMap.count("environ") ? ignoreMap.at("environ") : std::unordered_set<std::string>());
    ...
    auto filtered_logs = FeaturePruner::filterStrings(lowerlogs, ignoreMap.count("logs") ? ignoreMap.at("logs") : std::unordered_set<std::string>());
    ...
    if (access(prunedFolder.c_str(), F_OK) != 0) {
       ...
       LOGI(SCANNER_TAG, format_string("New folder created: %s", prunedFolder.c_str()));
    }
    ...
    if (!unfilteredCSV.is_open()) {
        LOGE(SCANNER_TAG, format_string("Failed to open unfiltered file: %s", unfilteredFile.c_str()));
    } else {
        ...
        for (size_t i = 0; i < lowerenviron.size(); ++i) {
             for (char ch : lowerenviron[i]) {
                 if (ch == '"') unfilteredCSV << "\"\""; else unfilteredCSV << ch;
             }
    ...
    if (!filteredCSV.is_open()) {
        LOGE(SCANNER_TAG, format_string("Failed to open filtered file: %s", filteredFile.c_str()));
    } else {
        ...
        for (size_t i = 0; i < filtered_environ.size(); ++i) {
            for (char ch : filtered_environ[i]) {
                 if (ch == '"') filteredCSV << "\"\""; else filteredCSV << ch;
             }
...
std::vector<std::string> FeatureExtractor::parse_proc_cgroup(pid_t pid, const std::string& delimiters) {
    ...
    while (std::getline(infile, line)) {
        std::vector<std::string> lineTokens = FeaturePruner::splitString(line, delimiters);
    ...
    for (size_t i = 0; i < content.size(); ++i) {
        if (content[i] == '\0') {
            if (i > start) {
                ...
                for (const auto& raw : FeaturePruner::splitString(arg, delimiters)) {
                    std::string cleaned;
                    for (char c : raw) {
                    ...
                    if (!cleaned.empty() && !FeaturePruner::isDigitsOnly(cleaned) && cleaned.size() > 1) {
...
std::vector<std::string> FeatureExtractor::parse_proc_map_files(pid_t pid, const std::string& delimiters) {
    ...
    while ((entry = readdir(dir)) != nullptr) {
        ...
        if (len != -1) {
            ...
            for (const auto& tok : FeaturePruner::splitString(target_str, delimiters)) {
                std::string simplified = FeaturePruner::normalizeLibraryName(tok);
                if (simplified.empty() || simplified.size() <= 1) continue;
                if (FeaturePruner::isDigitsOnly(simplified)) continue;
...
std::vector<std::string> FeatureExtractor::parse_proc_fd(pid_t pid, const std::string& delimiters) {
    ...
    DIR* dir = opendir(dir_path.c_str());
    if (!dir) {
        LOGE(SCANNER_TAG, format_string("Unable to open fd directory %s", dir_path.c_str()));
        return results;
    }
    struct dirent* entry;
    char link_target[PATH_MAX];
    while ((entry = readdir(dir)) != nullptr) {
        ...
        if (len != -1) {
            link_target[len] = '\0';
            std::string target_str(link_target);
            std::vector<std::string> tokens = FeaturePruner::splitString(target_str, delimiters);
            for (const auto& tok : tokens) {
                if (tok.empty()) continue;
                std::string cleaned = FeaturePruner::removeDatesAndTimesFromToken(tok);
...
std::vector<std::string> FeatureExtractor::parse_proc_environ(pid_t pid, const std::string& delimiters) {
    ...
    while (std::getline(in, entry, '\0')) {
        ...
        std::vector<std::string> tokens = FeaturePruner::splitString(entry, delimiters);
        for (auto& token : tokens) {
            ...
            if (FeaturePruner::isAllSpecialChars(token)) continue; 
...
std::vector<std::string> FeatureExtractor::parse_proc_exe(pid_t pid, const std::string& delimiters) {
    ...
    if (len != -1) {
        ...
        for (const auto& part : FeaturePruner::splitString(exePath, delimiters)) {
            if (!FeaturePruner::isDigitsOnly(part)) {
                out.push_back(part);
            }
        }
    } else {
        LOGE(SCANNER_TAG, format_string("Failed to readlink %s for PID %d", path.c_str(), pid));
    }
    return out;
}
...
std::vector<std::string> FeatureExtractor::readJournalForPid(pid_t pid, uint32_t numLines) {
    ...
    std::string cmd = "journalctl --no-pager -n " + std::to_string(numLines) + " _COMM=" + comm;
    ...
    for (char ch : cleaned_input) {
        if (delimSet.count(ch)) {
            if (!token.empty()) {
               ...
               if (!token.empty() && !FeaturePruner::isSingleCharToken(token) && !FeaturePruner::isDigitsOnly(token)) {
