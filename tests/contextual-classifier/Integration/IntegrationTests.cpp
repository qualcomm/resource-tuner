...
//
// One-file, mini.hpp-based integration suite for the Contextual Classifier.
// What we verify (PASS expected today):
//   - Classifier.InitPath: module registration → dlopen → cc_init() → netlink set_listen(true)
//   - Classifier.EventDelivery: spawn 'sleep 1' → event hits queue → worker starts classification
//
// What is marked XFAIL (expected to fail on your current Ubuntu box):
//   - FastText.ModelLoads_XFail: no model at /etc/classifier/fasttext_model_supervised.bin
//   - FastText.Predict_XFail: no prediction log (model missing)
//   - Cgroups.FocusedSliceMove_XFail: focused/app slice write fails
//
// Build (example):
//   g++ -std=c++17 -O2 IntegrationTests.cpp -o ContextualClassifierIntegrationTests
//
// Run (journal may require sudo):
//   sudo ./ContextualClassifierIntegrationTests
//
// Reports (auto via mini.hpp):
//   TEST_REPORT_DIR=./reports sudo ./ContextualClassifierIntegrationTests
//
// ---------------------------------------------------------------------
