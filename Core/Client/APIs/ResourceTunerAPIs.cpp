...
// Connection to the Resource Tuner Server.
...
int64_t tuneResources(int64_t duration, int32_t properties, int32_t numRes, SysResource* resourceList) {
    ...
    try {
        ...
        batch.setBuf(buf);
        batch.append<int8_t>(REQ_RESOURCE_TUNING)
             .append<int64_t>(0)
        ...
        if(batch.isBufSane()) {
            ...
            LOGE("RESTUNE_CLIENT", "Request Size exceeds max capacity [1KB]");
        }
...
int8_t retuneResources(int64_t handle, int64_t duration) {
    try {
        ...
        char buf[REQ_BUFFER_SIZE];
        batch.setBuf(buf);
        batch.append<int8_t>(REQ_RESOURCE_RETUNING)
...
int8_t untuneResources(int64_t handle) {
    try {
        ...
        char buf[REQ_BUFFER_SIZE];
        batch.setBuf(buf);
        batch.append<int8_t>(REQ_RESOURCE_UNTUNING)
             .append<int64_t>(handle)
             .append<int64_t>(-1)
             .append<int32_t>(0)
             .append<int32_t>(0)
...
int8_t getProp(const char* prop, char* buffer, size_t bufferSize, const char* defValue) {
    try {
        ...
        char buf[REQ_BUFFER_SIZE];
        batch.setBuf(buf);
        batch.append<int8_t>(REQ_PROP_GET) // Type
             .appendString(prop) // Prop / Key
             .append<uint64_t>(bufferSize); // Buffer Size (Result)

        if(batch.isBufSane()) {
            if(sendMsgHelper(buf) == 0) {
                // read the response
...
int64_t tuneSignal(uint32_t signalCode, int64_t duration, int32_t properties,
    ...
    try {
        ...
        VALIDATE_GE(numArgs, 0);
        VALIDATE_GE(properties, 0);

        char buf[REQ_BUFFER_SIZE];
        batch.setBuf(buf);
        batch.append<int8_t>(REQ_SIGNAL_TUNING) // Type
             .append<uint32_t>(signalCode) // SigCode
             .append<int64_t>(0) // Handle. 0 for tuneSignal
             .append<int64_t>(duration) // Duration
             .appendString(appName) // AppName / pkg
             .appendString(scenario) // Scenario
             .append<int32_t>(numArgs) // Number of Additional Args
             .append<int32_t>(properties) // Properties
             .append<int32_t>((int32_t)getpid()) // PID
             .append<int32_t>((int32_t)gettid()); // TID

        for(int32_t i = 0; i < numArgs; i++) {
            batch.append<uint32_t>(list[i]); // Additional Args, one at a time
        }
...
int8_t untuneSignal(int64_t handle) {
    try {
        ...
        char buf[REQ_BUFFER_SIZE];
        batch.setBuf(buf);
        batch.append<int8_t>(REQ_SIGNAL_UNTUNING)
             .append<uint32_t>(0)
             .append<int64_t>(handle)
             .append<int64_t>(-1)
             .appendString("")
...
int8_t relaySignal(uint32_t signalCode, int64_t duration, int32_t properties,
    ...
    try {
        ...
        char buf[REQ_BUFFER_SIZE];
        batch.setBuf(buf);
        batch.append<int8_t>(REQ_SIGNAL_RELAY)
             .append<uint32_t>(signalCode)
             .append<int64_t>(0)
             .append<int64_t>(duration)
             .appendString(appName)
             .appendString(scenario)
             .append<int32_t>(numArgs)
             .append<int32_t>(properties)
             .append<int32_t>((int32_t)getpid())
             .append<int32_t>((int32_t)gettid());
