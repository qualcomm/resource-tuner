...
typedef void (*WorkloadPostprocessCallback)(std::string &, uint32_t &, uint32_t &);
...
private:
	...
	static std::unordered_map<std::string, WorkloadPostprocessCallback> mWorkloadPostprocessCallbacks;
...
public:
	...
	std::unordered_map<std::string, WorkloadPostprocessCallback>& getWorkloadPostprocessCallbacks();
 ...
 * \param workloadPostprocessCallback A function Pointer to the Post processing Callback.
 *
 * \note This macro must be used in the Global Scope.
 */
#define URM_REGISTER_WORKLOAD_POSTPROCESS_CB(comm, workloadPostprocessCallback) \
        static Extensions CONCAT(_postprocess, comm)(comm, workloadPostprocessCallback);
